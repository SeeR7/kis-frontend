...
const ViewCell = ({ numerable, data, columns, count, setToggle, fullState, handleDelete, role, dep }: any) => {
  return (
    <tr key={data.id}>
      {numerable && <td key={"count"}>{count}</td>}
      {columns.map(({ value, accessor, type, selectData, link, addLink, label, accFrom, roles, deps }: any) => {
        if (!roles) {
          roles = ''
        }
        if (!deps) {
